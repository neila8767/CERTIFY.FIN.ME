...
export default function ChoixModeleDiplome() {
  ...
  useEffect(() => {
      ...
      console.log("ecoleId récupéré :", ecoleId);
     }, [token]);
  
    
  useEffect(() => {
    const url = `${API_BASE_URL}/ecoles-modeles`;
    ...
    axios.get(url)
      ...
      .catch(err => {
        console.error("❌ Erreur lors du GET /ecoles-modeles/modeles :", err);
        if (err.response) {
  ...
  const choisirModele = async (modeleId) => {
     const ecoleId = localStorage.getItem('ecole_id');
    try {
       await axios.put(`${API_BASE_URL}/ecoles-modeles/${ecoleId}/choix-modele`, { modeleId })
